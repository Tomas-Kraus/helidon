...
Data repository queries are an abstraction on top of Object–relational mapping (ORM). This
...
Data Repository module depends on Jakarta Persistence 3.1. Data Repository API module is included with this dependency.
...
Both Jakarta Persistence provider and Database JDBC driver must be added explicitly.
...
Database services share common config node root called `data`. Jakarta Persistence provider configuration should be
in `persistence-units.jakarta` sub-node of this root.

The Jakarta Persistence provider must be configured before you begin. In the example below we'll use Helidon Config to set up
Jakarta Persistence provider with EclipseLink and MySQL database as part of the persistence unit setup:

In this case there will be single persistence unit with default name. It's possible to add more persistence units into the array,
...
Another option is to use `DataSource`:

JDBC `DataSource` has its own `sources.sql` sub-node under common `data` root shared with persistence providers. Jakarta
Persistence provider configuration just references already configured `name` of the `DataSource`.
...
It's also possible to configure `DataSource` with default name without the `name` config option. Persistence provider
configuration must reference `"@default"` data source in that case.

== SE Application

Initialization of the data repository in  Java SE application is based on service registry. The only required
step to access the repository interface is to call corresponding `<T> T Services.get(Class<T> contract)` method:

[source,java]
----
PetRepository pets = Services.get(PetRepository.class);
----

Service registry will resolve the config structure and all the internal dependencies based on _persistence unit_
and `DataSource` names.

Data repository interface is linked with the default name of the configured _persistence unit_. Custom _persistence unit_ name
may be set using `@Data.PersistenceUnit` annotation:

[source,java]
----
@Data.Repository
@Data.PersistenceUnit("my-unit")
...
NOTE: `EntityManager` instance is valid only while `call` method is being executed. This instance can't be stored and used after
